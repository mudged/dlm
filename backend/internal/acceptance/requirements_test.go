...
// Tests trace to Gherkin in docs/requirements/acceptance-criteria.md (Given docs/requirements/requirements.md exists).

func requirementsPath(t *testing.T) string {
	t.Helper()
	// internal/acceptance -> repo root is ../.. from here; docs at ../../docs
	root := filepath.Join("..", "..", "..", "docs", "requirements", "requirements.md")
...
func TestAcceptance_REQ003_raspberryPiDeployment(t *testing.T) {
	...
	lower := strings.ToLower(doc)
	if !strings.Contains(lower, "docs/design/architecture.md") {
		t.Fatal("REQ-003 must require docs/design/architecture.md for deployment fit")
	}
...
func TestAcceptance_REQ011_restLightStateAPI(t *testing.T) {
	...
	if !strings.Contains(lower, "docs/design/architecture.md") {
		t.Fatal("REQ-011 must defer URL layout or defaults to docs/design/architecture.md")
	}
...
func TestAcceptance_REQ016_cameraResetModelAndSceneViews(t *testing.T) {
	...
	if !strings.Contains(lower, "docs/design/architecture.md") {
		t.Fatal("REQ-016 must reference docs/design/architecture.md for default framing")
	}
...
func TestAcceptance_REQ030_pythonSceneRandomHexColour(t *testing.T) {
	...
	archPath := filepath.Join(root, "docs", "design", "architecture.md")
	...
	if !strings.Contains(archStr, "random_hex_colour") {
		t.Fatal("docs/design/architecture.md must document scene.random_hex_colour (REQ-030)")
	}
}
...
func TestAcceptance_REQ029_highThroughputLightUpdates(t *testing.T) {
	...
	if !strings.Contains(lower, "docs/design/architecture.md") {
		t.Fatal("REQ-029 business rule 1 must require docs/design/architecture.md")
	}
...
func TestAcceptance_REQ021_sceneRoutinesPythonAndSceneAPI(t *testing.T) {
	...
	if !strings.Contains(archLower, "routineengine") || !strings.Contains(archLower, "python3") {
		t.Fatal("docs/design/architecture.md must document internal/routineengine and supervised python3 for REQ-021")
	}
	if !strings.Contains(archLower, "req-021") {
		t.Fatal("docs/design/architecture.md must reference REQ-021 for scene routines")
	}
	if !strings.Contains(archLower, "pyodide") {
		t.Fatal("docs/design/architecture.md must mention optional Pyodide for editor-only use (REQ-022)")
	}
	if !strings.Contains(archStr, "time.Ticker") {
		t.Fatal("docs/design/architecture.md must document time.Ticker for shape animation (REQ-021 / §3.17.2)")
	}
	if !strings.Contains(archLower, "headless") || !strings.Contains(archLower, "observer") {
		t.Fatal("docs/design/architecture.md must state headless execution and browser-as-observer (REQ-038)")
	}
}
...
func TestAcceptance_REQ032_threeSeededPythonSampleRoutines(t *testing.T) {
	...
	if !strings.Contains(archStr, "pythonRoutineSamples.ts") || !strings.Contains(archStr, "REQ-032") {
		t.Fatal("docs/design/architecture.md must name pythonRoutineSamples.ts and REQ-032 seed placement (business rule 7)")
	}
...
func TestAcceptance_REQ033_shapeAnimationRoutines(t *testing.T) {
	...
	if !strings.Contains(lower, "docs/design/architecture.md") {
		t.Fatal("REQ-033 rule 12 must require docs/design/architecture.md for persistence and simulation")
	}
	...
	if !strings.Contains(archLower, "req-033") || !strings.Contains(archLower, "3.17.2") {
		t.Fatal("docs/design/architecture.md must document REQ-033 and §3.17.2 shape animation")
	}
	if !strings.Contains(archLower, "definition_json") {
		t.Fatal("docs/design/architecture.md must document definition_json for shape routines (REQ-033)")
	}
}
