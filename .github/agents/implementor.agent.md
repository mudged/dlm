...
# Workflow
1. **Context Gathering:** If the session was bootstrapped from a work item (`docs/work-items/WI-*.md`), treat that file as the **primary scope** for this chat — follow its tasks, acceptance criteria, out-of-scope boundaries, and definition of done. You must still read `docs/architecture.md` and `docs/acceptance_criteria.md` for the REQ IDs and sections cited in the work item. If there is no work item, read those two docs in full before writing any code.
