...
2. The UI MUST consume or coordinate with the backend in a way defined in `docs/design/architecture.md` (deferred until architect pass).
...
1. `docs/design/architecture.md` MUST describe how the Go service and Next.js build fit the Pi 4 B deployment context after the architect pass.
...
- REQ-004 constrains packaging to a **single executable**; `docs/design/architecture.md` MUST be updated to reconcile Pi deployment (this REQ) with that constraint.
...
- In scope: Defining that the **primary deliverable** for a given OS/CPU (e.g. **linux/arm64** for Pi) is **one executable file** that fulfills the product’s runtime obligations (HTTP API and serving the UI per REQ-001), subject to how `docs/design/architecture.md` realizes that shape **within a single process/binary**.
...
4. `docs/design/architecture.md` MUST be updated after this requirement is accepted so that the **single-process / single-binary** model, Pi constraints (REQ-003), and Next.js + Tailwind behavior (REQ-001/002) are **consistent**.
...
1. The API MUST allow **querying** the **current** state of lights for a model: **all lights** in **one** response **and** the state of **one** light by **id** (exact resource layout deferred to **docs/design/architecture.md**).
...
6. **Default** state for lights in a newly created or legacy model without prior state MUST match **REQ-014** (all **off**, **100%** brightness, white **`#FFFFFF`** in canonical hex) and MUST be **consistent** across API and UI; **docs/design/architecture.md** MAY elaborate representation only.
...
4. After **camera reset**, the **visible** **framing** MUST match the **default** **defined** in **docs/design/architecture.md** for that **view** **type** (model vs scene), so **repeat** activations yield the **same** baseline (modulo viewport size).
...
3. While a routine **run** is **active**, any change to **on/off**, **hex colour**, or **brightness** for lights in that scene performed **by** **the** **routine** MUST be effected **through** **the** **scene** **spatial** **and** **bulk** **update** **semantics** **from** **REQ-020** **with** **REQ-011** **field** **semantics**, **implemented** **entirely** **inside** **the** **Go** **service** **process**. **`docs/design/architecture.md`** **MUST** **document** **the** **concrete** **process** **model** **(**supervised** **Python** **child**, **native** **shape** **simulation**, **HTTP** **vs** **in-process** **calls** **to** **the** **same** **validation** **surface** **,** **packages**, **and** **stop** **semantics** **)** **without** **contradicting** **this** **requirement**. **Per-light** **model** **APIs** **(**REQ-011**)** **remain** **valid** **for** **manual** **user** **or** **integrator** **actions** **outside** **routine** **automation**. **The** **browser** **MUST** **NOT** **execute** **production** **routine** **automation** **(**no** **in-browser** **interpreter** **or** **timer** **that** **applies** **routine** **light** **mutations** **)** **;** **the** **web** **UI** **observes** **authoritative** **server** **state** **(**REQ-038** **,** **REQ-041**)** **.
...
3. **`docs/design/architecture.md`**, **the** **REQ-022** **on-page** **reference**, **and** **the** **REQ-024** **API** **reference** **MUST** **list** **all** **three** **attributes** **with** **definitions** **of** **which** **world** **axis** **each** **maps** **to**.
...
6. **`docs/design/architecture.md`** **MUST** **describe** **the** **chosen** **three.js** **material**/**rendering** **approach** **for** **REQ-028** **(**including** **how** **brightness** **maps** **to** **emissive** **intensity** **or** **equivalent**)** **and** **how** **it** **fits** **Pi**/**WebGL** **constraints** **from** **REQ-003** **where** **relevant**.
...
3. **Connection reuse:** **`docs/design/architecture.md` SHOULD** document which **HTTP** features are enabled or assumed (for example **HTTP/2**, **HTTP/1.1 keep-alive**) and **client pooling** or equivalent guidance for integrators and for the shipped web UI.
4. For **low-latency** refresh or **many concurrent viewers**, **`docs/design/architecture.md` SHOULD** specify **server-push** (**SSE**, **WebSocket**, or **equivalent**) **or** justify **bounded polling** that still satisfies **REQ-012**-class timeliness under the stated load.
...
- In scope: A **single** **documented** **callable** **on** **the** **Python** **routine** **scene** **surface** **(**REQ-022**)** **that** **returns** **a** **string** **valid** **as** **`color`** **in** **REQ-011** **(**`#` **plus** **six** **hex** **digits**)** **for** **each** **call**, **using** **the** **statistical** **behavior** **defined** **in** **business** **rule** **2** **below**. **On-page** **reference** **(**REQ-024**)** **and** **`docs/design/architecture.md`** **MUST** **name** **the** **callable** **and** **show** **a** **short** **commented** **example** **consistent** **with** **REQ-024** **sample** **style**.
...
4. **CodeMirror** **completions** **(**REQ-022**)** **and** **any** **browser** **worker** **or** **authoring** **helper** **used** **for** **editor** **tooling** **MUST** **stay** **aligned** **with** **the** **chosen** **Python** **callable** **name** **and** **sync**/**`async`** **semantics** **documented** **in** **`docs/design/architecture.md`** **(**Python** **routine** **runtime** **and** **scene** **binding** **)** **.
...
6. **`docs/design/architecture.md`** **MUST** **describe** **where** **equivalence** **is** **evaluated** **(**client**, **server**, **both**)** **,** **what** **is** **cached**, **invalidation** **rules**, **and** **documented** **no-op** **update** **behavior** **(**including** **device** **push** **skips** **where** **applicable**)** **,** **in** **line** **with** **REQ-029** **observer**/**refresh** **strategy** **where** **relevant**.
...
7. **`docs/design/architecture.md`** **MUST** **name** **where** **the** **initial** **seed** **content** **is** **defined** **(**e.g.** **embedded** **strings** **or** **shipped** **files**)** **and** **how** **those** **definitions** **relate** **to** **REQ-025** **(**new** **blank** **routine** **template**)** **if** **applicable**.
...
8. **Overlap:** **When** **a** **light** **lies** **inside** **more** **than** **one** **active** **shape** **volume**, **the** **product** **MUST** **apply** **one** **deterministic** **precedence** **rule** **(**e.g.** **lowest** **shape** **index** **wins** **or** **highest** **z** **center** **—** **documented** **in** **`docs/design/architecture.md`** **)** **so** **colour** **assignment** **is** **stable** **and** **testable**.
...
12. **`docs/design/architecture.md`** **MUST** **describe** **persistence** **shape** **for** **definitions**, **simulation** **tick**, **speed** **(**SI** **m/s** **and** **any** **cm/s** **UI** **mapping**)** , **wrap**/**deflect** **details**, **overlap** **precedence**, **and** **how** **REQ-027** **applies** **to** **this** **kind** **(**no** **Python** **editor** **or** **REQ-024** **on** **this** **surface** **unless** **optional** **cross-links** **only**)**.
...
6. **`base_url`** **validation** **(**SSRF** **defence**)**: **Every** **device** **`base_url`** **accepted** **by** **the** **registry** **(**create** **and** **update** **paths**)** **MUST** **be** **a** **syntactically** **valid** **URL** **whose** **scheme** **is** **either** **`http`** **or** **`https`** **and** **whose** **host** **is** **non-empty** **after** **trimming** **whitespace**. **URLs** **with** **other** **schemes** **(**e.g.** **`file`**, **`ftp`**, **`gopher`**, **`javascript`**)**,** **schemes** **with** **no** **host** **component** **(**e.g.** **`http:`** **alone**)**,** **or** **inputs** **that** **fail** **to** **parse** **as** **a** **URL** **MUST** **be** **rejected** **with** **HTTP** **`400`** **and** **the** **documented** **JSON** **error** **envelope** **(**`{ "error": { "code": "invalid_base_url", "message": <human-readable>, "details"? } }`** **per** **architecture** **§3.2**)**. **Rejection** **MUST** **occur** **before** **any** **outbound** **HTTP** **request** **is** **attempted** **against** **the** **supplied** **address** **(**so** **the** **process** **cannot** **be** **coerced** **into** **probing** **arbitrary** **loopback** **ports** **or** **`file://`** **resources** **on** **the** **host**)**. **Implementations** **MAY** **normalize** **the** **stored** **value** **(**trim** **trailing** **`/`**, **round-trip** **through** **standard-library** **URL** **parsing**)** **so** **long** **as** **the** **logical** **destination** **is** **preserved**.
...
1. **Server-side execution:** Starting a routine (**REQ-021**) MUST cause **ongoing** **server-side** **automation** **on** **the** **backend** **until** **stop** **or** **failure** **,** **independent** **of** **any** **web** **client** **session** **(**process** **model** **per** **`docs/design/architecture.md`** **and** **REQ-021**)** **.
...
4. **`docs/design/architecture.md` MUST** **name** **the** **chosen** **push** **transport** **(**SSE** **vs** **WebSocket** **vs** **other**)** **,** **the** **URL** **shape** **or** **subscription** **pattern**, **delta** **message** **schema**, **reconnect** **and** **full** **resync** **behavior**, **and** **how** **this** **interacts** **with** **REQ-029** **and** **REQ-031**.
5. **Pi** **:** **The** **design** **MUST** **remain** **plausible** **on** **Raspberry** **Pi** **4** **constraints** **(**REQ-003**)** **(**connection** **count**, **message** **rate**, **JSON** **size**)**.
6. **Observability** **of** **fan-out** **failures** **:** **When** **the** **server** **emits** **a** **revision** **/** **delta** **payload** **for** **one** **topic** **(**e.g.** **a** **model**)** **but** **fails** **to** **enumerate** **the** **other** **topics** **that** **should** **receive** **the** **same** **change** **(**e.g.** **scenes** **containing** **that** **model**)**, **the** **fan-out** **failure** **MUST** **NOT** **be** **silently** **swallowed**. **The** **server** **MUST** **record** **the** **failure** **through** **its** **structured** **logger** **at** **`WARN`** **level** **or** **higher**, **including** **at** **minimum** **the** **scope** **identifier** **the** **lookup** **was** **performed** **for** **(**e.g.** **`model_id`**)** **and** **the** **wrapped** **underlying** **error**, **so** **that** **operators** **can** **observe** **when** **subscribers** **on** **dependent** **topics** **are** **about** **to** **stop** **receiving** **deltas** **until** **the** **next** **change** **or** **reconnect**. **Successful** **fan-out** **paths** **are** **not** **required** **to** **log** **at** **`WARN`** **;** **only** **the** **error** **branch** **is** **normative**.
...
- In scope: `docs/design/architecture.md` MUST list the canonical GOOS/GOARCH pairs, artifact naming (file name per target), and how the static UI is embedded in the same binary.
...
3. `docs/design/architecture.md` MUST cross-reference REQ-003 (Pi) and REQ-004 so deployment notes name which published artifact maps to Raspberry Pi (linux/arm64) and which to desktop Windows or Linux.
...
2. **Release:** A documented procedure (in `docs/engineering/` or `README.md` as appropriate for audience) MUST describe how to cut a version (tag) such that publish artifacts for windows/amd64, linux/amd64, and linux/arm64 attach to that release.
3. `docs/design/architecture.md` MUST summarize CI and release (workflow names, trigger events, artifact outputs) after the architect pass.
...
1. README and `docs/design/architecture.md` MUST state clearly that Python routine execution requires Python on the PATH (or documented absolute path policy) and that no other product runtime besides the binary is needed for non-Python features.
...
- Out of scope: Duplicating every contributor command from `docs/engineering/`; REQ-046 demands operator-facing coverage in README.md while advanced detail may stay in `docs/engineering/`.
...
6. The device MUST expose a **configured light count** usable by the sweep **even when unassigned**; **`docs/design/architecture.md`** MUST ensure the device registry (**REQ-035**) carries that count (or a documented derivation).
...
5. The OpenCV computer-vision functionality MUST NOT require operators to **install a separate Python runtime**. This **no-separate-Python-install** goal is **independent** of **REQ-045** (which governs only **user-authored** Python **routines**). The **mechanism** (native bindings, embedded/bundled runtime, or other) is **deferred** to **`docs/design/architecture.md`**, and MUST stay consistent with **single-binary** packaging (**REQ-004**) and **cross-platform** builds (**REQ-043**).
