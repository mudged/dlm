...
| REQ-004 | One executable file per release target; assets embedded (or generated inside the process); Docker/compose is not the canonical install path. |
...
| REQ-040 | §3.17, §3.17.2: after an accepted `POST …/stop`, no further routine-originated state mutations within ≤ 2 SI seconds (the `python3` child gets `SIGTERM`/`SIGKILL` after `T_force`; the shape `time.Ticker` is cancelled before the next tick). |
...
| REQ-043 | §3.4, §6.6: three canonical release targets (`GOOS`/`GOARCH`) — windows/amd64, linux/amd64, linux/arm64 (Pi 4 class); one executable per pair with the static UI embedded and documented artifact names. |
...
| REQ-045 | §6.9: the deployment host needs only the product binary for the baseline (API + embedded UI + SQLite + shape animation); `python3` on `PATH` (≥ a documented minimum — the implementor picks, e.g. 3.11+) is required only when running user Python scene routines (§3.17). |
| REQ-046 | §6.10, §3.7: `README.md` (operator audience) — download from GitHub Releases, choose the Pi (linux/arm64) artifact, `systemd` unit example, update flow (replace the binary, restart the service, preserve `DLM_DATA_DIR`/DB); no `REQ-*` identifiers in the README body; the developer `./scripts/run.sh` remains secondary (REQ-008) per `docs/engineering/` cross-links as needed. |
