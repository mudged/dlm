...
- **Framing (REQ-016):** Fit the camera to the §3.12 AABB `[0,0,0]` – `(Mmax+1)` per axis, plus a marker-radius margin, using the same `applyDefaultFraming` pattern as §4.7 but with bounds derived from scene-space positions `(sx,sy,sz)` for all lights. A "Reset camera" control on the scene canvas re-invokes that same fit (no API call).
